...
@pytest.mark.anyio
async def test_authenticated_user(registered_user: dict):
...
@pytest.mark.anyio
async def test_authenticated_user_not_found():
    with pytest.raises(security.HTTPException):
        await security.authenticate_user("test@example.net", "1234")


@pytest.mark.anyio
async def test_authenticated_user_wrong_password(registered_user: dict):
    with pytest.raises(security.HTTPException):
